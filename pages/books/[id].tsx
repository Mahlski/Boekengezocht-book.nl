...
export default function Home() {
  ...
  let productUrl = `https://api.rainforestapi.com/request?api_key=A0F0A693535D4422A196039C14B380F9&type=product&amazon_domain=amazon.in&gtin=${id}`;
  ...
  let getBooks = async () => {
    ...
    const productData = await makeRequest(productUrl, "GET");
    console.log(productData);
    if (
      productData.data["request_metadata"] != undefined &&
      productData.data["request_metadata"]["amazon_url"]
    )
      setAmazonUrl(productData.data["request_metadata"]["amazon_url"]);
    else setAmazonUrl("Unavailable");
  };
  ...
  return (
    <div className="bg-gray-50 pb-5">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        {book ? <ProductView book={book} /> : ""}
        <Tabs
          defaultActiveKey="wheretobuy"
          id="uncontrolled-tab-example"
          className="mb-3 mt-0"
        >
          <Tab eventKey="wheretobuy" title="Where to Buy">
            {!book ? (
              ""
            ) : (
              <div className="bg-white shadow overflow-hidden sm:rounded-lg grid grid-cols-2">
                <img
                  src="https://books2search.com/images/amazon.png"
                  width="35%"
                ></img>
                {amazonUrl == "Unavailable" ? (
                  ...
                  <a
                    className="btn-cust-green w-full hover:text-green-500"
                    onClick={() => window.open(amazonUrl)}
